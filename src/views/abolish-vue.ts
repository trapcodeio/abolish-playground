import { AbolishRule, ValidationError } from "abolish/src/Types";
import { reactive, readonly, Ref, ref, UnwrapRef, watch } from "vue";
import { extendRef, watchDebounced } from "@vueuse/core";
import { Abolish, Rule } from "abolish";

type ValidatedReactiveOptions = {
  async?: boolean;
  delay?: number | true;
  Abolish?: typeof Abolish;
};

type ValidatedRefOptions = ValidatedReactiveOptions & { name?: string };

function setToRef(ref: Ref<any>) {
  return (value: any) => {
    ref.value = value;
  };
}
...
export function vReactive<
  R extends Record<string | keyof T, AbolishRule>,
  T extends object
>(target: T, rules: R, options: ValidatedReactiveOptions = {}) {
  /**
...
>(target: T, rules: R, options: ValidatedReactiveOptions = {}) {
  ...
  return [original, error, validated] as [
    typeof original,
    typeof error,
    typeof validated
  ];
